...
import { Users, Settings, Gauge, Plus } from 'lucide-react';
    ...
    <section className="px-6 md:px-12 py-16 md:py-24 bg-[#050505] relative border-t border-white/5">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="bg-[#0a0a0a] border border-white/5 p-6 h-64 animate-pulse" />
          ))}
        </div>
      ) : allCars.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-zinc-500 font-mono">[ NO_CARS_AVAILABLE ]</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center gap-4 py-3 border-y border-zinc-900/50 mb-4 font-mono text-[8px] text-zinc-500 uppercase tracking-widest">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-cyan-900">{car.efficiency || 'HIGH'}</span>
                  </div>
                </div>
